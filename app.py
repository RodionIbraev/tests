...
def show_all_docs_info():
    docs_info = []
    for current_document in documents:
        docs_info.append(show_document_info(current_document))
...
def secretary_program_start():
    ...
    while True:
        ...
        if user_command == 'p':
            ...
            print(result[0])
            for doc_info in result[1]:
                print('{} "{}" "{}"'.format(doc_info[0], doc_info[1], doc_info[2]))
        elif user_command == 's':
            ...
            user_doc_number, user_shelf_number = move_doc_to_shelf()
